...
We can derive (Supplementary Material, Section 1) the theoretical honeymoon time, $T$, or the time until the effective reproduction number exceeds one, as

\begin{equation}
\begin{aligned}
  T  = \frac{1}{\mu} \log\left(\frac{v_0-v}{\vcrit-v}\right)
\end{aligned}
\label{eq:honeymoon-time-main}
\end{equation}

\noindent
where $\mu$ is the birth and death rate, $v_0$ is the pre-decline effective immunization coverage, $v$ is the post-decline effective immunization coverage, and $\vcrit = 1-1/\Rn$ represents the critical effective immunization coverage, or the herd-immunity threshold. In the case where the post-decline effective immunization coverage drops below the critical threshold (i.e., $v < \vcrit$), the ratio $\frac{v_0-v}{\vcrit-v}$ describes the ``buffer'' of existing immunity that can be lost before the effective reproduction number exceeds one, and $\frac{1}{\mu}$ characterizes how quickly these susceptibles will accumulate through births.

Honeymoon times are predicted to vary substantially, depending on the extent of immunization decline (Figure \ref{fig:simple-honeymoon}A, shown for a population with a life expectancy of 80 years and thus $\mu = 1/80$ years\textsuperscript{-1} and a pathogen like measles with $\Rn = 17$). When declines are dramatic, honeymoon times can be short (for example, less than a year if prior immunization levels are just above the herd immunity threshold). However, when declines are small, honeymoon times are predicted to be on the order of decades. For example, if the pre-decline effective immunization coverage is 96\%, honeymoon times are expected to be greater than 10 years as long as immunization does not drop below 80\%. Prior immunity in the population also has a substantial effect on honeymoon times; if pre-decline immunization is 95\% (instead of 96\%), a drop to 80\% immunization will yield a honeymoon time of 4.85 years.

We can also derive (Supplementary Material, Section 1) the relationship between pre-decline immunization, $v_0$, post-decline immunization, $v$, and theoretical honeymoon time, $T$:

\begin{equation}
%v_0 = e^{\mu t^H}(1-1/\Rn)+(1-e^{\mu t^H})v.
v_0=\bigl(1-e^{\mu T}\bigr)v+e^{\mu T}\vcrit.
...
\subsection{Theoretical honeymoon time}
\label{sec:methods-theoretical-honeymoon}

The observed ``post-honeymoon time'' will depend both on the susceptibility in the population and the frequency and timing of importations \citep{MetcalfEtAl2020}. Here we focused exclusively on the accumulation of susceptibles, defining the theoretical honeymoon time as the time until $\Reff=\Rn S(t)$ crosses 1. We chose this definition because it represents the time after which an importation would successfully lead to an outbreak (deterministically). It also has the advantage of being analytically tractable. In this section, we provide an overview of the model and honeymoon time derivation; additional details are provided in Supplementary Material Section 1. 

We derived theoretical honeymoon times in the simplest case with a basic SIR (Susceptible-Infected-Recovered) model. We assumed a constant population size with equivalent birth and death rates, where immunization occurs at birth. We defined the ``effective immunization coverage'' as the proportion of new births that are succesffully immunized, thus accounting for both vaccine coverage and efficacy. The model is defined by the following system of differential equations:

\begin{align}
\dd{S}{t} &= \mu (1-v)-\beta SI-\mu S \\ 
\dd{I}{t} &= \beta SI-(\gamma-\mu)I \\ 
\dd{R}{t} &= \mu v+\gamma I-\mu R
\label{eq:sir-main-methods}
\end{align}

\noindent
where $\mu$ is the birth and death rate, $v$ is the effective immunization coverage, $\beta$ is the transmission rate, and $\gamma$ is the recovery rate. In this model $\vcrit = 1-1/\Rn$ is the critical immunization coverage, or the herd-immunity threshold. If $v>\vcrit$, the pathogen will go locally extinct. 

We considered a population with some ``pre-decline'' effective immunization coverage, $v_0 > \vcrit$, such that measles is locally extinct. We then assumed that immunization coverage drops to some ``post-decline'' level, $v < \vcrit$, such that the pathogen can reemerge. In this case, with $v < \vcrit < v_0$, the accumulation of susceptible individuals is given by
...
\noindent
Then, the theoretical honeymoon time $T$, or the time until $\Reff=\Rn S(t)=1$, is 
