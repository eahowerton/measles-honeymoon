...
Here, we only consider the first case where a population is above the herd immunity threshold, $1/\Rn$, because otherwise there is no honeymoon period. We are interested in the duration of the honeymoon period, which we hypothesize will depend on a number of factors including the $\Rn$ of the pathogen, the equilibrium number of susceptibles $S^*$, the birth rate $\mu$, the immunization rate $v$ under decline, and the timing of importations. Because we cannot control the timing of importations, we define the honeymoon period as the time it takes for $\Reff = \Rn \cdot S(t) > 1$. In other words, the time it takes for $\Reff$ to cross 1 is a lower bound on the honeymoon period, where any importation after this point will start an outbreak.
...
\begin{figure}[h]
    ...
    \caption{Theoretical honeymoon time predictions, computed as time until Re > 1, under five assumptions about age-structured mixing. Results are shown for a population with $\mu =1/80 \text{ years}^{-1}$, a pathogen with $\Rn = 17$, and a pre-decline immunization of 95\% and 96\%.
}
\end{figure}
