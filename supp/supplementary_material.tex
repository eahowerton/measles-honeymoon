...
\author{Emily Howerton, Ayesha Mahmud, David J. Earn, \\Bryan T. Grenfell, C. Jessica E. Metcalf}
...
\begin{align*}
    ...
    \frac{dR_{i\neq1}}{dt} &= \alpha_{i-1} R_{i-1} + \gamma R_i - \mu R_i - \alpha_i R_i 
\end{align*}
...
$$
    K = \frac{\beta}{\gamma + \mu} \begin{bmatrix}
    ...
    \end{bmatrix} = \frac{\beta}{N(\gamma + \mu)} \theta \overrightarrow{S(t)},
    $$
...
We consider a metapopulation model with $P$ identical patches that are assumed to be equally connected, such that $1-c\%$ of transmission arises from cross-patch contact. The model can be defined as 

% \begin{align*}
%     \frac{dS_p}{dt} &= \mu N (1-v_p) - \frac{\beta}{N} \Big((1-c) I_p + \sum_{q\neq p} \frac{c}{(P-1)} I_q \Big) S_p - \mu S_p \\
%     \frac{dI_p}{dt} &= \frac{\beta}{N} \Big((1-c) I_p + \sum_{q\neq p} \frac{c}{(P-1)} I_q \Big)  S_p - (\gamma + \mu) I_p \\
%     \frac{dR_p}{dt} &= \mu N v_p + \gamma I_p - \mu R_p
% \end{align*}
...
\begin{figure}[h]
    ...
    \caption{Sensitivity of results to the timing of infection introduction. \textbf{[ADD HERE]}}
\end{figure}
