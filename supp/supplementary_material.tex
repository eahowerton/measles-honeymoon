...
\david{I've introduced macros for reproduction numbers and typeset them in what has become standard notation.}

\section{Theoretical honeymoon time in the SIR model}
We consider here how reductions in vaccination can lead to rebounds in disease prevalence in a SIR (Susceptible, Infected, Recovered) model. In particular, we will focus on the post-vaccination ``honeymoon", or how long it takes for the disease to rebound after vaccination starts declining. We use the term ``effective immunization coverage" throughout for the proportion successfully immunized, thereby accounting for both vaccination coverage and imperfect vaccine efficacy.

To begin, we will consider the simplest case where (a) a population has been under a ``pre-decline'' vaccination regime for a sufficiently long period of time to be at equilibrium, and (b) that the pathogen is locally eliminated.  Let $v_0$ be the effective immunization coverage during the period of herd immunity, and let $v$ be the effective immunization coverage after the decline begins.

In the SIR model, we know there are two possible equilibria: 
\begin{enumerate}
    \item disease-free equilibrium, where $S^* = v_0$ \david{No, the DFE is $1-v_0$ (assuming you're taking $S$ to mean proportion susceptible).}
    \item endemic equilibrium, where $S^* = \frac{1}{\beta}(\mu + \gamma + \frac{\mu(\mu + \gamma)}{\sigma})$
      \david{No, the EE for the standard SIR model is $S^* = \frac{1}{\Rn} = \frac{\gamma+\mu}{\beta}$.  The extra term does not come from the SIR model and you haven't defined $\sigma$.  It is for the SEIR model with $1/\sigma$ being the mean latent period.  Even then it is better to write $S^*=\frac{1}{\Rn}$ and $\Rn=(\frac{\beta}{\gamma+\mu})(\frac{\sigma}{\sigma+\mu})$.   It is also confusing to say there are two equilibria, but in the first case in the presence of immunity and the second case without.  I strongly suggest you actually write down the model equations before stating what the equilibria are, and derive equilibria for one clear model.}
\end{enumerate}

Here, we consider only the disease-free case in which the pre-decline effective immunization coverage is above the herd-immunity threshold, $v_0>1-1/\Rn$, equivalently $S^*=1-v_0<1/\Rn$; otherwise there is no positive honeymoon period. We are interested in the duration of the honeymoon period, which we hypothesize will depend on a number of factors including the $\Rn$ of the pathogen, the equilibrium proportion susceptible $S^*$, the birth rate $\mu$, the post-decline effective immunization coverage $v$, and the timing of importations. Because the timing and establishment of importations cannot be predicted by this deterministic model, we define the theoretical honeymoon time as the time until $\Reff=\Rn S(t)$ crosses 1. This provides a lower bound on the actual rebound time, because rebound additionally requires an importation to arrive and establish.

\david{I did some editing above, but I suggest a wholesale replacement from here until the thick red line.  Between the thick red line and the start of the age-structured model section, I've retained your former text that I suggest be deleted.}

We can calculate analytically the time at which $\Reff$ crosses 1 under the assumptions of local extinction ($I=0$), constant post-decline effective immunization coverage $v$, and a constant population scaled to 1.  Births and deaths then occur at the same per-capita rate $\mu$, and the proportion susceptible satisfies
\begin{equation}
...
We can calculate analytically the time it takes for $\Reff$ to cross 1, under assumptions of local extinction (i.e., $I = 0$) and a constant post-decline immunization rate, $v$. In this case, we have a differential equation for the number of susceptibles, $S$, which we can solve using the following steps:
...
\noindent
We can then find the time $t$ at which $\Reff = \Rn S(t) = 1$, which yields

% \begin{align}
%     (1-v) + (S_0 - (1-v)) e^{-\mu t} &= 1/\Rn  \\
%     (S_0 - (1-v))e^{-\mu t}  &= (1/\Rn - (1-v)) \\\
%     e^{-\mu t}  &= \frac{1/\Rn - (1-v)}{S_0 - (1-v)} \\
% \end{align}
$$
t = \frac{-1}{\mu} \log \Big(\frac{1/\Rn - (1-v)}{S_0 - (1-v)}\Big)
$$
...
% \begin{align}
%     t &= \frac{-1}{\mu} \log \Big(\frac{1/\Rn - (1-v)}{(1-v_0) - (1-v)}\Big) \\
%     - t \mu &= \log \Big(\frac{1/\Rn - (1-v)}{(1-v_0) - (1-v)}\Big) \\
%     e^{- t \mu} &= \Big(\frac{1/\Rn - (1-v)}{(1-v_0) - (1-v)}\Big) \\
%     (1-v_0) - (1-v) &= e^{t \mu} (1/\Rn - (1-v)) \\
%     v - v_0 &= e^{t \mu} (v - 1 + 1/\Rn) \\
%     v_0 &= v - e^{t \mu} (v - 1 + 1/\Rn) \\
%     v_0 &= v(1 - e^{t \mu}) + e^{t \mu} (1 - 1/\Rn)
% \end{align}
