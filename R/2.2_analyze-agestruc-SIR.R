...
for(i in 1:nrow(tst_release_full)){
  ...
  IC_manual = vax_equilib_long %>% filter(start_vax == tmp_start_vax) %>% arrange(age)
...
# save results
saveRDS(susc_after_release_long_full, "data/output-data/susc_after_release_long.rds")
saveRDS(rt_after_release_full_long, "data/output-data/rt_after_release_long.rds")
...
ggsave("figures/honeymoon_age_full_start95.pdf", width = 3.5, height = 2.5)
...
ggsave("figures/honeymoon_age_full_comparestart.pdf", width = 9, height = 3)
