...
for(i in 1:nrow(rt_release)){
  ...
  paras_tmp = paras_all_noimport[[tmp_waifw_id]]
...
# saveRDS(rt_after_release_twopatch_long, "data/rt_after_release_twopatch_long_fixphi1.rds") # rt_after_release_twopatch_long_fixphi1.rds is the final version (for now)
...
honeymoon_period %>% filter(start_vax == 0.96, round(release_vax1,3) %in% c(0.9)) %>%
  ggplot(aes(x = release_vax2, y = min_time, color = as.factor(waifw_id), alpha = as.factor(phi))) +
  geom_line(linewidth = 0.8) +
  facet_grid(cols = vars(release_vax1), labeller = label_both) +
...
ggsave("figures/honeymoon_time_by_WAIFW_twopatch.pdf", width = 6, height = 5)
